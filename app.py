...
def compile_graph_aot(arch):
    ...
    def taichi_add(in_ten: ti.types.ndarray(), out_ten: ti.types.ndarray(), addend: ti.i32):
        for i, j, k in in_ten:
            out_ten[i, j, k] = ti.cast(ti.cast(in_ten[i, j, k], ti.i32) + addend, ti.i8)

    in_ten = ti.ndarray(dtype=ti.i8, shape=(320, 320, 4))
    out_ten = ti.ndarray(dtype=ti.i8, shape=(320, 320, 4))
    in_ten.from_numpy(np.ones((320, 320, 4), dtype=np.uint8))
    out_ten.from_numpy(np.ones((320, 320, 4), dtype=np.uint8))
    addend = 3

    arg_0 = ti.graph.Arg(ti.graph.ArgKind.NDARRAY, "in_ten", dtype=ti.i8, field_dim=3)
    arg_1 = ti.graph.Arg(ti.graph.ArgKind.NDARRAY, "out_ten", dtype=ti.i8, field_dim=3)
